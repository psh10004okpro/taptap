...
- `IapResult.gems` — 서버 검증본이 있으면 **그 수량으로 지급**한다.
  클라이언트 `GEM_PACKS` 와 서버 `verify-purchase` 의 PRODUCTS 사본이 어긋나면
  원장(gem_ledger)에 기록된 것과 다른 양을 주게 되므로, 클라 값은 폴백일 뿐이다.
  (두 사본을 함께 갱신하라는 규칙은 그대로 유효하다 — 이건 그 규칙이 깨졌을 때의 안전망이다.)
- `grantGems` 는 지급의 단일 관문이므로 여기서 음수/NaN/Infinity 를 거른다.
  소수는 내림. 어떤 경로(광고/구매/치트)로 들어와도 잔액과 VIP 누적이 오염되지 않는다.
  E2E: "보석 지급 관문".
